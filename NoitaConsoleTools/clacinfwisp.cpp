...
vector<vector<vector<int>>> infwispCache;//使用三维向量保证线程安全
bool WriteCache = false;
extern bool clearCache;

clacPar clacA;

int getSwitchNum(const char* str) {
    while (true) {
        printf("%s", str);
        string commondStr;
        ...
        else if (commondStr == "2") {
            return 2;
        }
        else if (commondStr == "3") {
            return 3;
        }
        else {
            printf("你输入的参数不是1、2或3，请重新输入\n");
        }
    }
}
                    ...
                    for (int arc = arc2; arc <= arcLT; arc++)
                    {
                        for (int nul = nul2; nul <= nulLT; nul++)
                        {
                            for (int dec = 0; dec <= decLT; dec++)
                            {
                                ...
                                {
                                    YouNeedNum = -(280 * nul + 75 * add + 25 * pin + 50 * hel + 80 * arc - dec * 42 - spl * 30);
                                    if (YouNeedNum < 0 && out) {
                                        test = startNum / 30;
                                        if (test == 1) { out = 0; }
                                    ...
                                    if (YouNeedNum >= startNum + 1 && YouNeedNum <= endNum + 1) {//符合条件就是可以永久化的，+1是为了排除一些不合条件的选项
                                        ...
                                        if (isSaveOrNo) {
                                            ...
                                            HasAnwThread[IDPlus]++;
                                            infwispCache[IDPlus].push_back(vector<int>(0));
                                            infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(add);
                                            infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(pin);
                                            infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(hel);
                                            infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(arc);
                                            infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(dec);
                                            infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(spl);
                                            infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(nul);
                                        }
                                        else {
                                            mtx.lock();
                ...
                for (int hel = hel2; hel <= helLT; hel++)
                {
                    for (int arc = arc2; arc <= arcLT; arc++)
                    {
                        for (int nul = nul2; nul <= nulLT; nul++)
                        {
                            ...
                            for (int dec = 0; dec <= decLT; dec++)
                            {
                                YouNeedNum = -(280 * nul + 75 * add + 25 * pin + 50 * hel + 80 * arc - dec * 42);
                                if (YouNeedNum < 0 && out) {
                                    test = startNum / 42;
                                    if (test == 1) { out = 0; }
                                ...
                                if (YouNeedNum >= startNum + 1 && YouNeedNum <= endNum + 1) {//符合条件就是可以永久化的，+1是为了排除一些不合条件的选项
                                    ...
                                    if (isSaveOrNo) {
                                        ...
                                        infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(arc);
                                        infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(dec);
                                        infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(splLT);
                                        infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(nul);
                                    }
                                    else {
                                        mtx.lock();
                ...
                for (int hel = hel2; hel <= helLT; hel++)
                {
                    for (int arc = arc2; arc <= arcLT; arc++)
                    {
                        for (int nul = nul2; nul <= nulLT; nul++)
                        {
                            ...
                            for (int spl = 0; spl <= splLT; spl++)
                            {
                                YouNeedNum = -(280 * nul + 75 * add + 25 * pin + 50 * hel + 80 * arc - spl * 30);
                                if (YouNeedNum < 0 && out) {
                                    test = startNum / 30;
                                    if (test == 1) { out = 0; }
                                ...
                                if (YouNeedNum >= startNum + 1 && YouNeedNum <= endNum + 1) {//符合条件就是可以永久化的，+1是为了排除一些不合条件的选项
                                    ...
                                    if (isSaveOrNo) {
                                        ...
                                        infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(hel);
                                        infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(arc);
                                        infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(decLT);
                                        infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(spl);
                                        infwispCache[IDPlus][HasAnwThread[IDPlus] - 1].push_back(nul);
                                    }
                                    else {
...
int isFileCustOrNo = 0;
int closeNum = 1;
    void clacinfwisp::clac(const cfgClass& cfg) {//addLT蓝表,pinLT乒乓,decLT红表，helLT螺旋魔弹,arcLT相位弧度,splLT连锁法术
        printf("注:乒乓回弹和盘旋魔弹影响的存在时间数值一样、相位传送和真正环绕(True Orbit)同理\n\n");
        int startNum, endNum, modMax, addLT, pinLT, decLT, helLT, arcLT, splLT, YouNeedNum, isSaveOrNo = 0;
        int test = 1, nulLT = 0;
        int Count = 0;
        int threadNum = 1;
        WriteCache = false;
        ...
        infwispCache = vector<vector<vector<int>>>(threadNum);
        HasAnwThread = vector<int>(threadNum);
        while (true)
        {
            ...
            int SiwtchNum = getSwitchNum("输入1直接打印到控制台，输入2将写入文件，输入3将数据存入缓存，在计算完成后可以使用inflist指令读取:");
            switch (SiwtchNum)
            {
            case 1: {
                break;
            ...
            case 3: {
                WriteCache = true;
                clearCache = false;
                printf("tips:接下来将不会在控制台内打印数据，而是将数据暂时存入内存中\n");
                break;
            }
            }
                                        ...
                                        for (int spl = 0; spl <= splLT; spl++)
                                        {
                                            ...
                                            if (YouNeedNum >= startNum + 1 && YouNeedNum <= endNum + 1) {//符合条件就是可以永久化的，+1是为了排除一些不合条件的选项
                                                ...
                                                if (isSaveOrNo) {
                                                    ...
                                                    HasAnwThread[0]++;
                                                    infwispCache[0].push_back(vector<int>(0));
                                                    infwispCache[0][HasAnwThread[0] - 1].push_back(add);
                                                    infwispCache[0][HasAnwThread[0] - 1].push_back(pin);
                                                    infwispCache[0][HasAnwThread[0] - 1].push_back(hel);
                                                    infwispCache[0][HasAnwThread[0] - 1].push_back(arc);
                                                    infwispCache[0][HasAnwThread[0] - 1].push_back(dec);
                                                    infwispCache[0][HasAnwThread[0] - 1].push_back(spl);
                                                    infwispCache[0][HasAnwThread[0] - 1].push_back(nul);
                                                }
                                                else {
                                                    mtx.lock();
                                                    printf("蓝表数量:%d ,乒乓数量:%d ,螺旋魔弹数量:%d ,相位弧度数量:%d ,红表数量:%d ,连锁法术数量:%d ,无害射击数量:%d \n", add, pin, hel, arc, dec, spl, nul);
                                    ...
                                    for (int dec = 0; dec <= decLT; dec++)
                                    {
                                        ...
                                        if (YouNeedNum >= startNum + 1 && YouNeedNum <= endNum + 1) {//符合条件就是可以永久化的，+1是为了排除一些不合条件的选项
                                            ...
                                            if (isSaveOrNo) {
                                                ...
                                                infwispCache[0][HasAnwThread[0] - 1].push_back(arc);
                                                infwispCache[0][HasAnwThread[0] - 1].push_back(dec);
                                                infwispCache[0][HasAnwThread[0] - 1].push_back(splLT);
                                                infwispCache[0][HasAnwThread[0] - 1].push_back(nul);
                                            }
                                            else {
                                                mtx.lock();
                                    ...
                                    for (int spl = 0; spl <= splLT; spl++)
                                    {
                                        ...
                                        if (YouNeedNum >= startNum + 1 && YouNeedNum <= endNum + 1) {//符合条件就是可以永久化的，+1是为了排除一些不合条件的选项
                                            ...
                                            if (isSaveOrNo) {
                                                ...
                                                infwispCache[0][HasAnwThread[0] - 1].push_back(hel);
                                                infwispCache[0][HasAnwThread[0] - 1].push_back(arc);
                                                infwispCache[0][HasAnwThread[0] - 1].push_back(decLT);
                                                infwispCache[0][HasAnwThread[0] - 1].push_back(spl);
                                                infwispCache[0][HasAnwThread[0] - 1].push_back(nul);
                                            }
                                            else {
    ...
    void clacinfwisp::clacNoInput(const cfgClass& cfg,int startNum, int endNum, int SiwtchNum, int addLT, int pinLT, int decLT, int helLT, int arcLT, int splLT, int nulLT,string fileP) {//addLT蓝表,pinLT乒乓,decLT红表，helLT螺旋魔弹,arcLT相位弧度,splLT连锁法术
        printf("注:乒乓回弹和盘旋魔弹影响的存在时间数值一样、相位传送和真正环绕(True Orbit)同理\n\n");
        int YouNeedNum, isSaveOrNo = 0;
        int test = 1;
        int Count = 0;
        int threadNum = 1;
        WriteCache = false;
        ...
        while (true)
        {
            ...
            if (isSaveOrNo) {
                file.open(File, ios::out | ios::app);
                file << "本次输入的存在时间范围为:" << startNum << "到" << endNum << endl;
                file << "蓝表上限为:" << addLT << endl;
                file << "乒乓上限为:" << pinLT << endl;
                file << "螺旋魔弹上限为:" << helLT << endl;
                file << "相位上限为:" << arcLT << endl;
